...
public class NodeServerResource extends StorageItemServerResource {
    ...
    public void checkItemAccess() throws ResourceException {
        final Node requestedNode = getNode(getCurrentPath(), VOS.Detail.max, 0);
        ...
        if (writeGroupStr == null) {
            throw new ResourceException(new SecurityException("User can't write to node."));
        } else {
            writeResponse(Status.SUCCESS_OK,
                          new JSONRepresentation() {
                              ...
                              public void write(final JSONWriter jsonWriter) throws JSONException {
                                  jsonWriter.object()
                                            .key("writeGroup").value(writeGroupStr)
                                            .key("readGroup").value(readGroupStr)
                                            .endObject();
                              }
                          });
        }
